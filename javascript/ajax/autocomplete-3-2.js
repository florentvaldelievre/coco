...
var _inputField2=null; // le champ texte lui-m�me
var _inputField3=null; // le champ texte lui-m�me
var _submitButton=null; // le bouton submit de notre formulaire

function initAutoComplete(form,field,submit){
  _documentForm=form;
  _inputField=field[0]; // villedepart
  _inputField2=field[1]; //cpdepart
  _inputField3=field[2]; //paysdepart

  _inputField4=field[3]; // villearrive
  _inputField5=field[4]; //cparrive
  _inputField6=field[5]; //paysarrive
     
  _submitButton=submit;
  _inputField.autocomplete="off";
  _inputField2.autocomplete="off";
  _inputField3.autocomplete="off";
  _inputField4.autocomplete="off";
  _inputField5.autocomplete="off";
  _inputField6.autocomplete="off";  
...
var _adresseRecherche = "formulaires/autocompletion.php" //l'adresse � interroger pour trouver les suggestions
...
function traiteXmlSuggestions(xmlDoc) {
  ...
  var infoville = xmlDoc.getElementsByTagName('infoVille');
  var maxResult = 10;

var optionsListe=new Array(infoville.length);
	for (i=0; i <infoville.length; ++i)
		 optionsListe[i]=new Array(3);


  for (var i=0; i < infoville.length; ++i) {
      
      var childInfoVille=xmlDoc.getElementsByTagName('infoVille')[i].childNodes; //les fils pour chaque 'infoVille'
      
	      for (var j=0; j<childInfoVille.length; ++j) {
	
	     // alert("["+i+"]"+"["+j+"]"+childInfoVille[j].firstChild.nodeValue);
	      optionsListe[i][j] = childInfoVille[j].firstChild.nodeValue;
...
function metsEnPlace(valeur, liste){
  ...
  for(var f=0; f<liste.length; ++f){
    ...
    var suggestionStr = liste[f].join(' - ');

    nouveauSpan.innerHTML=suggestionStr; // le texte de la suggestion
...
var divOnMouseDown=function(){
	var suggestionStr = getSuggestion(this);
	var suggestionArray = suggestionStr.split(' - ');
  _inputField.value=suggestionArray[0];
  _inputField2.value=suggestionArray[1];
